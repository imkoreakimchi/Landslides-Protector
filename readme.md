...
> Node 22 환경에서 `npm WARN npm does not support Node.js v22.x` 경고가 발생했다면
> 현재 npm이 6.x와 같이 오래된 버전인 경우입니다. `npm install -g npm@latest`
> 혹은 Node 20 LTS 재설치를 통해 npm 10.x 이상으로 업데이트한 뒤 진행하세요.
...
2. **프런트엔드**
   ...
   npm run dev
   ```
   개발 서버는 `0.0.0.0:5173`에 바인딩되므로 동일 네트워크의 다른 기기에서도
   `http://<your-ip>:5173`으로 접속할 수 있습니다.
